...
const analyticsGoalsConfig = {
  goals: [
    {
      name: "Email Capture",
      type: "event",
      condition: "email_capture",
      value: 5.00,
      description: "User subscribes to newsletter"
    },
    {
      name: "Affiliate Click - High Value",
      type: "event", 
      condition: "affiliate_click",
      value: 2.00,
      description: "Click on affiliate link >€30 commission"
    },
    {
      name: "Content Engagement",
      type: "event",
      condition: "page_view_time",
      value: 0.50,
      description: "User reads for 2+ minutes"
    },
    {
      name: "Social Share",
      type: "event",
...
async function implementAnalytics() {
  ...
  // 5. Genera script di testing
  await createAnalyticsTestScript();
  
  console.log('🎉 Analytics implementation completata!');
  console.log('');
  console.log('📋 PROSSIMI PASSI:');
  console.log('1. Testa con: node scripts/test-analytics.js');
  console.log('2. Verifica in Google Analytics Real-Time');
  console.log('3. Setup Goals in GA4 Interface');
  console.log('4. Configura Hotjar account (opzionale)');
}
    ...
    <div class="dashboard">
        ...
        <div class="chart-container">
            ...
            <div id="goalsContainer">
                ${analyticsGoalsConfig.goals.map(goal => `
                    <div class="goal-item">
                        <span class="status-indicator status-active"></span>
                        <strong>${goal.name}</strong> - €${goal.value}
                        <br><small>${goal.description}</small>
                    </div>
                `).join('')}
            </div>
        </div>
        ...
        <div class="chart-container">
            ...
            <div id="topContent">
                <p>Loading content performance...</p>
            </div>
        </div>
...
// 🧪 SCRIPT DI TESTING
async function createAnalyticsTestScript() {
  const testScript = `
...
module.exports = { testAnalyticsImplementation, createTestServer };`;
  ...
  fs.writeFileSync(path.join(__dirname, 'test-analytics.js'), testScript);
  console.log('✅ Test script creato');
}
...
if (require.main === module) {
  implementAnalytics().then(() => {
    ...
    console.log('');
    console.log('📋 VERIFICA IMPLEMENTAZIONE:');
    console.log('1. node scripts/test-analytics.js');
    console.log('2. Apri: http://localhost:8000');
    console.log('3. Verifica Google Analytics Real-Time');
    console.log('');
    console.log('📊 DASHBOARD: http://localhost:8000/analytics-dashboard.html');
    console.log('');
    console.log('🎯 GOOGLE ANALYTICS:');
    console.log('- Real-Time Report: https://analytics.google.com');
    console.log('- Events: Verifica "affiliate_click" e "email_capture"');
    console.log('');
